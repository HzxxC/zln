...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		$this->assign('date_week', $this->getNowDate());
		$this->assign('main_nav', $this->getNav(1));
		// var_dump($this->getNav(1));
	...
	private function getNowDate() {
		...
		return array('date' => date('Y年m月d日', time()), 'week' => $week[$w]);
	}
	...
	private function getNav($id) {
		...
		foreach ($navs as $key => $nav) {
			...
			$navs[$key]['children_navs'] = $children_navs;
		}
